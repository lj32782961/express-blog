...
async function sendMessageToAPI(message) {
    const modelName = "gemini-2.0-flash"//"gemini-1.5-flash";
...
async function loadSettings() {
    try {
        ...
        if (!response.ok) {
            console.error('获取秘钥数据失败:', response.status, response.statusText);
            isSettingsLoaded = false;
            return; // 退出函数，不再尝试解析 JSON
        }

        const config = await response.json();
        if (config.settings_values) {
            keys = config.settings_values;
            keys = keys.split(',');
